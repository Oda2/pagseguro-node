...
pagseguro.prototype.sendTransaction = function (transaction, cb) {
  ...
  if (this.checkoutData.paymentMethod === 'creditCard') {
    ...
    let cpfCnpj = this.holder ? this.holder.cpf_cnpj : this.sender.cpf_cnpj;
    if (cpfCnpj.length === 11) {
      this.checkoutData.creditCardHolderCPF = cpfCnpj;
    } else {
      this.checkoutData.creditCardHolderCNPJ = cpfCnpj;
    }
  }
